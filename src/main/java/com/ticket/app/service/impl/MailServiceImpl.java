        ...
        for (Map.Entry<Ticket, Integer> entry : purchase.getTicket().entrySet()) {
            if (k != 0) {
                ...
                br.append("Поздравляем вас с покупкой билетов на мероприятие ").append("\n")
                        .append(entry.getKey().getEvent().getName()).append("\n").append("Номер заказа: ")
                        .append(entry.getKey().getEvent().getId()).append("\n\n").append("Дата и время :").append(entry.getKey().getEvent().getDate()).append("\n")
                        .append("Место проведения: ").append(entry.getKey().getEvent().getClubName()).append("\n")
                        .append("По адресу: ").append(entry.getKey().getEvent().getAddress()).append("\n\n")
                ...
                try {
                    ...
                    mimeMessageHelper.setSubject("Билеты на " + entry.getKey().getEvent().getName());
