...
public class AbstractVKontakteOperations {
    ...
    protected void requireAuthorization() {
        if (!isAuthorized) {
            throw new MissingAuthorizationException("org/springframework/social/vkontakte");
        }
    }
    ...
    protected <T> VKArray<T> deserializeArray(VKGenericResponse response, Class<T> itemClass) {
        ...
        for (int i = 1; i < items.size(); i++) {
            try {
                ...
                throw new UncategorizedApiException("org/springframework/social/vkontakte", "Error deserializing: " + items.get(i), e);
            }
        }
